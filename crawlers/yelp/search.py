...
def getRowFromBusiness(business):
  name=url=phone=rating=review_count=address=is_closed=''
  if ('name' in business):
   name=business['name'].encode('utf-8')
  if ('url' in business):
    url=business['url'].encode('utf-8')
  if ('phone' in business):
    phone=business['phone'].encode('utf-8')
  if ('rating' in business):
    rating=business['rating']
  if ('review_count' in business):
    review_count=business['review_count']
  if ('display_address' in business['location']):
  ...
  categories=''
  i = 0
  for index, category in business['categories']:
    if i > 0:
      categories += ', '
    categories += index.encode('utf-8')
    i += 1

  return [name, url, phone, rating, review_count, address, is_closed, categories]

#print json.dumps(response, sort_keys=True, indent=2)

with open(fname, 'a') as outf:
	outcsv = csv.writer(outf)
	#outcsv.writerow(['Name', 'URL', 'Phone', 'Rating', 'Number of reviews', 'Address', 'Is Closed', 'Category List'])
	for business in response['businesses']:
		try:
			outcsv.writerow(getRowFromBusiness(business))
		except KeyError:
			pass
